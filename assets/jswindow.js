...
function jswindow(args) {
  ...
  function clipRight() {
    return (
      parseInt(vWindow.outerWindow.style.width) +
        parseInt(vWindow.outerWindow.style.left) +
        borderThickness * 2 >
      window.innerWidth
    );
  }
  function clipBottom() {
    return (
      parseInt(vWindow.outerWindow.style.height) +
        parseInt(vWindow.outerWindow.style.top) +
        borderThickness * 2 >
      window.innerHeight
    );
  }
  ...
  this.open = function (args) {
    vWindow.outerWindow.style.width =
      args && args.width ? args.width + "px" : defaultWidth + "px";
    vWindow.outerWindow.style.height =
      args && args.height ? args.height + "px" : defaultHeight + "px";
    vWindow.outerWindow.style.left =
      args && args.left
        ? args.left + "px"
        : randInt(
            0,
            window.innerWidth - parseInt(vWindow.outerWindow.style.width)
          ) + "px";
    vWindow.outerWindow.style.top =
      args && args.top
        ? args.top + "px"
        : randInt(
            0,
            window.innerHeight - parseInt(vWindow.outerWindow.style.height)
          ) + "px";
    vWindow.outerWindow.style.maxWidth =
      window.innerWidth -
      (parseInt(vWindow.outerWindow.style.left) + borderThickness * 2) +
      "px";
    vWindow.outerWindow.style.maxHeight =
      window.innerHeight -
      (parseInt(vWindow.outerWindow.style.top) + borderThickness * 2) +
      "px";
  ...
  if (args && args.icon) {
    ...
    vWindow.outerWindow.bar.icon.style.backgroundImage =
      'url("' + args.icon + '")';
  ...
  vWindow.outerWindow.bar.wname.appendChild(
    document.createTextNode(args && args.title ? args.title : "")
  );
  vWindow.outerWindow.bar.wname.classList.add("windowtitle");

  vWindow.outerWindow.bar.close = document.createElement("span");
  vWindow.outerWindow.bar.close.appendChild(
    document.createTextNode(String.fromCharCode(10006))
  );
  ...
  vWindow.outerWindow.addEventListener(
    "mousedown",
    function (e) {
      var allwindows = Array.from(document.querySelectorAll("div.window"));
      if (
        allwindows.indexOf(vWindow.outerWindow) != allwindows.length - 1 &&
        e.target != vWindow.outerWindow.bar.close
      )
        document.body.appendChild(vWindow.outerWindow);
    },
    false
  ...
  vWindow.outerWindow.bar.addEventListener(
    "mousedown",
    function (e) {
  ...
  function windowDrag(e) {
    e.preventDefault();
    vWindow.outerWindow.style.left =
      vWindow.outerWindow.offsetLeft - (oldcursorX - e.clientX) + "px";
    oldcursorX = e.clientX;
    vWindow.outerWindow.style.top =
      vWindow.outerWindow.offsetTop - (oldcursorY - e.clientY) + "px";
    oldcursorY = e.clientY;
  }
  ...
  function windowDragEnd() {
    ...
    vWindow.outerWindow.style.left =
      Math.min(
        Math.max(
          vWindow.outerWindow.offsetLeft,
          0 - parseInt(vWindow.outerWindow.style.width) + leftClip
        ),
        window.innerWidth - rightClip
      ) + "px";
    vWindow.outerWindow.style.top =
      Math.min(
        Math.max(vWindow.outerWindow.offsetTop, topClip),
        window.innerHeight - bottomClip
      ) + "px";
    if (!clipRight())
      vWindow.outerWindow.style.maxWidth =
        window.innerWidth -
